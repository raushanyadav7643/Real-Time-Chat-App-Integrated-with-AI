...
const ChatContainer = () => {
  const { messages, selectedUser, setSelectedUser, sendMessage, getMessages, editMessage, deleteMessage, votePoll, isProfileOpen, setIsProfileOpen } =
  ...
  return selectedUser ? (
    <div className={`h-full flex flex-col relative bg-transparent overflow-hidden max-w-4xl mx-auto w-full transition-all duration-500 ${isDark ? 'border-x border-white/5' : 'border-x border-black/5 shadow-2xl bg-white'} ${isProfileOpen ? "hidden md:flex" : "flex"}`}>
      {/* Header */}
      <div
        onClick={() => setIsProfileOpen(!isProfileOpen)}
        className={`flex-none flex items-center gap-2 md:gap-4 px-3 md:px-5 backdrop-blur-2xl z-20 min-h-[85px] md:min-h-0 shadow-xl transition-all duration-500 cursor-pointer ${isDark ? 'border-b border-white/5 bg-[#0f172a]/95' : 'border-b border-slate-100 bg-white/95'}`}
        style={{ paddingTop: 'calc(24px + env(safe-area-inset-top))', paddingBottom: '16px' }}
      >
        <button
          onClick={(e) => {
            e.stopPropagation();
          ...
          className={`p-2 rounded-full md:hidden transition-all active:scale-95 ${isDark ? 'text-gray-400 hover:bg-white/10 hover:text-white' : 'text-slate-500 hover:bg-black/5 hover:text-slate-900'}`}
        >
      ...
      {/* Messages */}
      <div className={`flex-1 overflow-y-auto px-3 py-4 md:px-4 md:py-6 scroll-smooth custom-scrollbar flex flex-col gap-4 md:gap-5 transition-all duration-500 ${isDark ? 'bg-[#0b0d14]/60 backdrop-blur-sm' : 'bg-slate-50/50'}`}>
      ...
      {/* Input area */}
      <div className={`flex-none p-4 pb-safe md:p-6 backdrop-blur-3xl z-20 transition-all duration-500 ${isDark ? 'bg-[#020617]/40 border-t border-white/5' : 'bg-white border-t border-slate-100 shadow-t-xl'}`}>
        {editingMessage && (
        ...
        <div className="flex items-center gap-2 md:gap-4 relative">
          ...
          <div className={`flex-1 flex items-center border rounded-[2rem] px-3 md:px-5 py-1 md:py-1.5 relative shadow-inner group transition-all focus-within:ring-2 ring-indigo-500/20 ${isDark ? 'bg-[#111827] border-white/5' : 'bg-slate-100 border-slate-200'}`}>
            {isRecording ? (
              ...
              <>
                <button ref={emojiButtonRef} onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`p-1.5 md:p-2 transition-transform active:scale-90 ${showEmojiPicker ? 'text-indigo-500' : 'text-gray-500 hover:text-gray-300'}`}>
                ...
                <div className="flex items-center gap-1 md:gap-2 relative">
                  ...
                  <button ref={attachmentButtonRef} onClick={() => setShowAttachmentMenu(!showAttachmentMenu)} className={`p-1.5 md:p-2.5 rounded-full transition-all active:scale-95 ${showAttachmentMenu ? 'bg-indigo-600 text-white shadow-xl rotate-45' : (isDark ? 'text-gray-500 hover:text-white hover:bg-white/5' : 'text-slate-400 hover:bg-slate-200 hover:text-slate-600')}`}>
                  ...
                  <button onClick={startRecording} className={`p-1.5 md:p-2.5 transition-all active:scale-90 ${isDark ? 'text-gray-500 hover:text-indigo-500' : 'text-slate-400 hover:text-indigo-600'}`}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 0 1-3-3V4.5a3 3 0 1 1 6 0v8.25a3 3 0 0 1-3 3Z" /></svg></button>
                </div>
              </>
            )}
          </div>
          <button onClick={handleSendMessage} className={`flex-none bg-gradient-to-tr from-indigo-500 to-purple-600 p-3 md:p-4 rounded-full text-white shadow-xl active:scale-90 transition-all group overflow-hidden relative ${isDark ? 'shadow-indigo-500/50' : 'shadow-indigo-500/30'}`}>
